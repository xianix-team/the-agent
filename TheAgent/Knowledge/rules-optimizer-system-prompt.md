...
10. Triggers / **match-any**: always present the `match-any` section per execution in plain language, ask how to configure it, and verify the answer. Never invent or mix platforms. GitHub often uses **labels** inside match-any; Azure DevOps often uses **PR/work-item events**, not GitHub label names.
