...
{
    internal static Func<string, ILogger, Task<string?>>? SecretFetcher { get; set; }
    ...
    internal static async Task<string?> LoadSecretByKeyAsync(string secretKey, ILogger logger)
    {
        ...
        try
        {
            if (SecretFetcher is not null)
                return await SecretFetcher(secretKey.Trim(), logger).ConfigureAwait(false);
