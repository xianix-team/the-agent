...
description: Register/ping (GitHub) or manual URL handoff (ADO); evidence from tool fields; end Completed or Failed.
---

# Connection

This skill teaches the **workflow**. `RegisterGitHubRepositoryWebhook` performs GitHub register+ping.
...
4. Do **not** require `GITHUB-WEBHOOK-SECRET` for registration. Do **not** set a GitHub
   hook `config.secret`. Do **not** auto-write `github-webhook-verification-secret` into Rules.
