...
| `instruction_hardening.py` | Executor-owned system prompt, canary, and PreToolUse policy (no SDK dependency) |
| `system_prompt_hardening.md` | Hardened system-prompt append (fail-closed if missing from the image) |
...
| `PROMPT` | Yes | Fully interpolated Claude Code prompt to execute. Webhook-derived placeholders are wrapped in `<user_data>` tags by the agent before this env is set. When `XIANIX_INPUTS` includes a non-empty `platform`, the executor prepends a short `[Xianix host context]` block (platform + optional `repository-name`) so Claude Code / plugins can pick the right host APIs without relying on env alone. The executor always appends a hardened system prompt (Claude Code preset + `system_prompt_hardening.md`) and installs a PreToolUse hook. |
| `XIANIX-HARDENING-AUDIT` | No | **Operator-only.** When `1`/`true`/`yes`, tool-policy denials are logged but not enforced. Default is blocking (fail closed). Seeded only from the agent host (`EXECUTOR-HARDENING-AUDIT`); **cannot** be set via tenant `with-envs` / webhook inputs. Use only in isolated test environments — never in production. |
...
### Tool permissions and PreToolUse hardening

The Claude Code SDK runs with `permission_mode=bypassPermissions` because executor containers are unattended — there is no operator to answer interactive tool-approval prompts. That does **not** mean tools run unchecked:

1. **Fail closed on hook availability** — if the SDK's `HookMatcher` is missing (version skew / import failure), the run refuses to start.
2. **PreToolUse gate** — `instruction_hardening.evaluate_tool_use` denies environment/credential exfiltration, secret-path reads, canary leakage in tool args, and (in report-only mode) `git commit`/`push`.
3. **System prompt + canary** — `system_prompt_hardening.md` states the trust hierarchy; a per-run canary is injected and any observation of it aborts the run.

Keeping the SDK's interactive permission UI enabled alongside the hook is not viable in this environment; the PreToolUse policy is the permission layer. Do not remove `bypassPermissions` without an alternate non-interactive approval path.

`XIANIX-HARDENING-AUDIT` softens PreToolUse denials to log-only. It is intentionally **not** part of the tenant trust boundary: the agent host must set `EXECUTOR-HARDENING-AUDIT`, and any `with-envs` entry for that name is ignored with a warning.
