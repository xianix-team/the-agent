...
#
# `maven` and `gradle` are the one exception, and a principled one: every other
# ecosystem ships its build tool inside the runtime (the .NET SDK carries
# MSBuild, node carries npm, go carries the go tool, rust carries cargo, python
# carries pip), so installing the runtime is enough. The JDK ships no build tool
# at all, which would leave every Maven/Gradle repo unbuildable.
ALLOWED_TOOLS="${XIANIX_RUNTIME_ALLOWED_TOOLS:-bun deno dotnet elixir erlang go gradle java kotlin maven node python ruby rust scala swift zig}"
...
# per-runtime logic, and the set (not a single name) matters: mise's `maven`
# brings no JDK, so `mvn` on its own dies with "JAVA_HOME is not defined
# correctly" — it has to be installed alongside `java`.
BIN_ALIASES="cargo:rust rustc:rust rustup:rust javac:java jar:java mvn:java,maven gradle:java,gradle kotlinc:kotlin"
...
command_not_found_handle() {
    ...
    # progress output be the evidence of an actual download.
